...
// 以下为心知天气的 获取天气-API（简体中文 / 英文），复制到浏览器地址栏中即可查看（需申请个人账号，免费版每个账号可以请求 20次/分钟）
// http://api.seniverse.com/v3/weather/daily.json?key=S6eMmAGubL0Twlnxo&location=hefei&language=zh-Hans&unit=c&start=0&days=5
// http://api.seniverse.com/v3/weather/daily.json?key=S6eMmAGubL0Twlnxo&location=hefei&language=en&unit=c&start=0&days=5
#define WEB_SERVER "api.seniverse.com"
#define WEB_PORT "80"
#define WEB_PATH "/v3/weather/daily.json?key=S6eMmAGubL0Twlnxo&location=hefei&language=zh-Hans&unit=c&start=0&days=5"
...
static void http_get_task(void *pvParameters)
{
    ...
    struct in_addr *addr;
    int s, r;
    //char recv_buf[2048];//char recv_buf[64];
    char *http_rmsg;
    int max_recv_byte = 1024*4;  // socket单次接收的最大字节数
    ...
    while(1) {
        ...
        cjson_parse_xinzhi_weather(json_http); // cjson解析心知天气返回的报文
