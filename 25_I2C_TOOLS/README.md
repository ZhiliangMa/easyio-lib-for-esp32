...
通过运行本例程，将`ESP32`作为`I2C总线调试器主机`（与Linux下的I2C调试工具有几分相似），来辅助调试I2C总线上的设备。用`终端`作为显示窗口，可进行 `搜索I2C总线上的设备`、`读/写I2C消息` 等操作，对`开发调试`及`编写I2C设备驱动`十分友好。
...
* `i2cget -c 0x68 -r 0x75 -l 1`：读取器件地址为 0x68的设备的 0x75 寄存器的内容，读取长度为1字节。（0x68是MPU6050/9250的器件地址，0x75是`WHO_AM_I`寄存器地址。MPU6050可以读出 0x68，MPU9250可以读出 0x71）
...
3. 默认引脚为：`SCL - 16`，`SDA - 17`。可在`cmd_i2ctools.c`中修改。
