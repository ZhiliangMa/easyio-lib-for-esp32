...
void jpg_lcd_spi_dma_task(void* arg)
{
    ...
    // 计算JPEG解码耗费的时间
    int64_t t_start, t_end=0;
    while(1) {
        // 以`SPI-DMA 双缓冲环形队列`的方式去刷液晶屏，解码jpg图片让lcd往复显示两张图片。

        t_start = esp_timer_get_time(); //获取当前系统时间。计数1000是1ms，1s是1000000
        // 解码 image_esp32mcu.jpg，输出RGB565像素格式到内存 pimg
        ret=jpg_decode(image_esp32mcu_jpg_start, &pimg, 0);
        ESP_ERROR_CHECK(ret);
        t_end = esp_timer_get_time(); //获取当前系统时间。计数1000是1ms，1s是1000000
        ESP_LOGI(TAG, "time:%lld us", t_end-t_start);
