...
extern const uint8_t image_apple_jpg_start[] asm("_binary_image_apple_jpg_start");
extern const uint8_t image_apple_jpg_end[] asm("_binary_image_apple_jpg_end");

extern const uint8_t image_guy_jpg_start[] asm("_binary_image_guy_jpg_start");
extern const uint8_t image_guy_jpg_end[] asm("_binary_image_guy_jpg_end");

extern const uint8_t image_wave_jpg_start[] asm("_binary_image_wave_jpg_start");
extern const uint8_t image_wave_jpg_end[] asm("_binary_image_wave_jpg_end");

static const char *TAG = "jpeg";

// jpg_lcd_spi_dma_effect_task 任务。初始化 SPI、LCD、jpg解码器，以`SPI-DMA 双缓冲环形队列`的方式去刷液晶屏，连续显示一张图片的波动动效。
void jpg_lcd_spi_dma_effect_task(void* arg)
{
    ...
    ret=jpg_decode(image_wave_jpg_start, &pimg, 0);
    ESP_ERROR_CHECK(ret);
    ...
    while(1) {
        ...
        lcd_spi_dma_display_pretty_colors(LCD_SPI, &pimg);
    }
}
...
void app_main(void)
{
    ...
    // 创建 jpg_lcd_spi_dma_effect_task 任务。
    xTaskCreate(jpg_lcd_spi_dma_effect_task, "jpg_lcd_spi_dma_effect_task", 1024*10, NULL, 3, NULL);
}
